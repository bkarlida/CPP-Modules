...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	else if (energyPoints > 0 && hitPoints <  maxHp && hitPoints > 0)
	{
		energyPoints -= 1;
		...
		{
			hitPoints += amount;
			std::cout << "ClapTrap " << name << " renewed itself " << amount << " points and ClapTrap "
			<< name <<"'s new hitPoints = " << hitPoints << std::endl;
		}
	}
	else if (hitPoints == 0)
	{
